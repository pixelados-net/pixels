...
var Module = fx.Module(
	"realm-messenger",
	fx.Provide(NewStore, NewPrivateChatWriter, NewService, delivery.New, profile.NewPresence, profile.NewUpdates, NewProfileBroadcaster),
	fx.Invoke(RegisterConnectionHandlers, profile.RegisterPresence, profile.RegisterRelationships, profile.RegisterUpdates, chatlog.RegisterLifecycle),
)
...
func NewService(config Config, store messengerrecord.Store, players playerservice.Manager, livePlayers *playerlive.Registry, rooms *roomlive.Registry, permissions permissionservice.Checker, redisClient *redis.Client, filter *chatfilter.Service, messageLog *chatlog.Writer) *messengercore.Service {
	...
	return messengercore.New(messengercore.Options{
		...
		FriendCacheTTL: config.FriendCacheTTL,
...
type HandlerDeps struct {
	...
	Log *zap.Logger
	// Groups reads social-group profile entries.
	Groups *groupmembership.Service
	// Players resolves exact usernames for public-profile compatibility.
...
func RegisterConnectionHandlers(handlers *realmconn.Handlers, dependencies HandlerDeps) {
	...
	session.RegisterHandlers(handlers.Inbound, session.Handler{Messenger: dependencies.Messenger, Delivery: dependencies.Delivery, Groups: dependencies.Groups, Players: dependencies.Players}, dependencies.Log)
	...
	social.RegisterHandlers(handlers.Inbound, social.Handler{Messenger: dependencies.Messenger, Delivery: dependencies.Delivery, Events: dependencies.Events, Translations: dependencies.Translations, Directory: dependencies.Players}, dependencies.Log)
	session.RegisterPrivacyHandlers(handlers.Inbound, session.PrivacyHandler{Messenger: dependencies.Messenger, Delivery: dependencies.Delivery, Events: dependencies.Events}, dependencies.Log)
}
