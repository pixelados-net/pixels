...
import (
	...
	invisituser "github.com/niflaot/pixels/networking/inbound/navigator/browse/visituser"
...
func RegisterHandlers(registry *netconn.HandlerRegistry, handler Handler, log *zap.Logger) {
	...
	_ = registry.Register(invisituser.Header, func(connection netconn.Context, packet codec.Packet) error {
		payload, err := invisituser.Decode(packet)
		if err != nil {
			return err
		}
		return handler.HandleVisitUser(context.Background(), connection, payload.Username)
	})
}
