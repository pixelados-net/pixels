...
// ListCards returns complete durable friend projections in one query.
func (repository *Repository) ListCards(ctx context.Context, playerID int64) ([]messengermodel.Card, error) {
	rows, err := repository.executor.Query(ctx, `select p.id,p.username,case when pp.gender='M' then 0 else 1 end,pp.look,coalesce(f.category_id,0)::int,pp.motto,f.relation,pp.block_following from messenger_friendships f join players p on p.id=f.friend_player_id join player_profiles pp on pp.player_id=p.id where f.player_id=$1 and p.deleted_at is null order by p.id`, playerID)
	if err != nil {
		return nil, fmt.Errorf("list player %d messenger friend cards: %w", playerID, err)
	}
	defer rows.Close()
	cards := make([]messengermodel.Card, 0)
	for rows.Next() {
		var card messengermodel.Card
		if err = rows.Scan(&card.ID, &card.Username, &card.Gender, &card.Look, &card.CategoryID, &card.Motto, &card.Relation, &card.BlockFollowing); err != nil {
			return nil, fmt.Errorf("scan messenger friend card: %w", err)
		}
