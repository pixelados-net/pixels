...
func (service *Service) Cards(ctx context.Context, playerID int64) ([]messengermodel.Card, error) {
	cards, cached := service.cachedCards(ctx, playerID)
	if cached {
		service.projectCardPresence(cards)
		return cards, nil
	}
	cards, err := service.loadDurableCards(ctx, playerID)
	if err != nil {
		return nil, err
	}
	service.cacheCards(ctx, playerID, cards)
	service.projectCardPresence(cards)
...
func (service *Service) Profile(ctx context.Context, viewerID int64, targetID int64) (Profile, error) {
	...
	result.IsFriend, err = service.store.IsFriend(ctx, viewerID, targetID)
	if err != nil {
		return Profile{}, err
	}
...
func (service *Service) cardFromRecord(record playerservice.Record, relation messengermodel.Relation, categoryID *int64) messengermodel.Card {
	card := durableCardFromRecord(record, relation, categoryID)
...
// durableCardFromRecord maps persistent player data without volatile presence.
func durableCardFromRecord(record playerservice.Record, relation messengermodel.Relation, categoryID *int64) messengermodel.Card {
	card := messengermodel.Card{ID: record.Player.ID, Username: record.Player.Username, Look: record.Profile.Look, Motto: record.Profile.Motto, Relation: relation, BlockFollowing: record.Profile.BlockFollowing}
...
func (service *Service) Remove(ctx context.Context, actorID int64, friendIDs []int64) ([]int64, error) {
	...
	removed, err := service.store.RemoveFriends(ctx, actorID, friendIDs)
	if err == nil && len(removed) > 0 {
		service.invalidateCards(ctx, append([]int64{actorID}, removed...)...)
	}
	return removed, err
}
...
func (service *Service) SetRelation(ctx context.Context, actorID int64, friendID int64, relation messengermodel.Relation) error {
	...
	service.invalidateCards(ctx, actorID)
	return nil
}
