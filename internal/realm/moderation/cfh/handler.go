...
type Handler struct {
	...
	*moderationruntime.Context
	// Photos validates and submits legacy photo evidence.
	Photos *camerareport.Service
}
...
func Register(registry *netconn.HandlerRegistry, runtime *moderationruntime.Context, photos *camerareport.Service) {
	handler := Handler{Context: runtime, Photos: photos}
	...
	_ = registry.Register(inselfie.Header, handler.selfie)
...
// selfie validates and submits legacy photo evidence.
func (handler Handler) selfie(connection netconn.Context, packet codec.Packet) error {
	payload, err := inselfie.Decode(packet)
	if err != nil {
		return err
	}
	actorID, err := handler.Actor(connection)
	...
	_, reportErr := handler.Photos.SubmitSelfie(context.Background(), actorID, payload.ExtraData, int64(payload.RoomID), int64(payload.TopicID), payload.Message)
	code, message := int32(0), handler.Text("moderation.report.received")
	if reportErr != nil {
		code, message = handler.reportError(reportErr)
	}
