...
func (handler Handler) guideDuty(connection netconn.Context, packet codec.Packet) error {
	...
	trackAllowed, err := handler.CanGuide(context.Background(), actorID)
	if err != nil {
		return err
	}
	guideAllowed = guideAllowed && trackAllowed
	...
	if active && handler.Events != nil {
		_ = handler.Events.Publish(context.Background(), bus.Event{Name: guideenrolled.Name, Payload: guideenrolled.Payload{PlayerID: actorID}})
	}
