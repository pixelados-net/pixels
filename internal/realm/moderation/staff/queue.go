...
func (handler Handler) closeIssues(connection netconn.Context, packet codec.Packet) error {
	...
	for _, issueID := range payload.IssueIDs {
		...
		result, _ := issueClosePacket(payload.Resolution, handler.Text("moderation.report.closed"))
...
func (handler Handler) closeDefault(connection netconn.Context, packet codec.Packet) error {
	...
	for _, issueID := range payload.IssueIDs {
		...
		result, _ := issueClosePacket(3, handler.Text("moderation.report.closed"))
		_ = handler.SendTo(context.Background(), issue.ReporterPlayerID, result)
	}
	return nil
}

// issueClosePacket creates the native closure notification rather than the report-submitted result packet.
func issueClosePacket(resolution int32, message string) (codec.Packet, error) {
	return outissueclose.Encode(resolution, message)
}
