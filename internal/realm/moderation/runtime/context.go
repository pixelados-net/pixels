...
type Context struct {
	...
	// GuideEligibility resolves the optional progression track gate.
	GuideEligibility GuideEligibility
	// GuideMinimumLevel stores the configured helpers-track minimum.
	GuideMinimumLevel int32
}

// GuideEligibility resolves paid progression talent levels.
type GuideEligibility interface {
	// MeetsGuideLevel reports whether one player satisfies the minimum.
	MeetsGuideLevel(context.Context, int64, int32) (bool, error)
}

// SetGuideEligibility configures the optional progression-backed guide gate.
func (runtime *Context) SetGuideEligibility(eligibility GuideEligibility, minimum int32) {
	runtime.GuideEligibility = eligibility
	runtime.GuideMinimumLevel = minimum
}

// CanGuide reports whether the optional progression gate permits guide duty.
func (runtime *Context) CanGuide(ctx context.Context, playerID int64) (bool, error) {
	if runtime.GuideMinimumLevel <= 0 || runtime.GuideEligibility == nil {
		return true, nil
	}
	return runtime.GuideEligibility.MeetsGuideLevel(ctx, playerID, runtime.GuideMinimumLevel)
}
