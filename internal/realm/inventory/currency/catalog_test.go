...
func TestLoadCatalogValidatesProjectDefinitions(t *testing.T) {
	root := filepath.Join("..", "..", "..", "..")
	catalog, err := LoadCatalog(Config{CatalogPath: filepath.Join(root, "currency", "types.json"), LedgerTypes: []int32{-1}})
	if err != nil {
		t.Fatalf("load catalog: %v", err)
	}
	translations, err := i18n.LoadCatalog(i18n.Config{Path: filepath.Join(root, "i18n", "translations.json")}, nil)
	if err != nil {
