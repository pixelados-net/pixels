...
from players where lower(username)='milo' and not exists (select 1 from bots where name='Party Bot');

insert into bots (owner_player_id, behavior_type, name, motto, figure, gender, chat_auto, chat_random)
select id, 'bartender', 'Frank', 'Need a drink?', 'hr-893-45.hd-180-1.ch-255-64.lg-285-64.sh-290-64', 'M', false, false
from players where lower(username)='milo' and not exists (select 1 from bots where name='Frank');

insert into bots (owner_player_id, behavior_type, name, motto, figure, gender, chat_auto, chat_random)
select id, 'visitor_log', 'Connie', 'I remember every visitor.', 'hr-515-33.hd-600-1.ch-635-70.lg-695-82.sh-730-62', 'F', false, false
from players where lower(username)='milo' and not exists (select 1 from bots where name='Connie');
...
from players where lower(username)='milo' and exists (select 1 from rooms where id=100 and is_bundle_template)
