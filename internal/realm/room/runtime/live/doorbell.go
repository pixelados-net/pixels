...
func (room *Room) occupancyLocked() Occupancy {
	...
	for playerID, occupant := range room.occupants {
		...
		if !occupant.Spectator {
			count++
		}
	}

	return Occupancy{RoomID: room.snapshot.ID, CategoryID: room.snapshot.CategoryID, Count: count, MaxUsers: room.snapshot.MaxUsers, PlayerIDs: playerIDs}
}
