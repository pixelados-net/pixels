...
// TestTeleportNearUsesWalkableUnoccupiedNeighbor verifies WIRED-safe direct movement resolution.
func TestTeleportNearUsesWalkableUnoccupiedNeighbor(t *testing.T) {
	room, err := NewRoom(Snapshot{ID: 9, MaxUsers: 128})
	if err != nil {
		t.Fatal(err)
	}
	item := worldfurniture.Item{ID: 11, Point: pointForTest(t, 2, 0), Definition: worldfurniture.Definition{Width: 1, Length: 1, StackHeight: 1}}
	if err = room.LoadWorld(WorldConfig{Grid: gridForTest(t, "000", 0, 0), Furniture: []worldfurniture.Item{item}, Door: worldpath.Position{Point: pointForTest(t, 0, 0)}}); err != nil {
		t.Fatal(err)
	}
	if _, err = room.Join(occupantForTest(7)); err != nil {
		t.Fatal(err)
	}
	moved, err := room.TeleportUnit(7, item.Point, worldunit.RotationSouth, true, TeleportNear)
	if err != nil || moved.Position.Point != pointForTest(t, 1, 0) {
		t.Fatalf("expected safe neighbor, moved=%#v err=%v", moved, err)
	}
	if _, err = room.TeleportUnit(7, pointForTest(t, 0, 0), worldunit.RotationSouth, false); err != nil {
		t.Fatal(err)
	}
	if _, err = room.AddEntity(-1, 1, worldunit.KindBot, worldpath.Position{Point: pointForTest(t, 1, 0)}, worldunit.RotationSouth); err != nil {
		t.Fatal(err)
	}
	if _, err = room.TeleportUnit(7, item.Point, worldunit.RotationSouth, true, TeleportNear); !errors.Is(err, worldpath.ErrInvalidGoal) {
		t.Fatalf("expected occupied alternatives to fail, got %v", err)
	}
}
