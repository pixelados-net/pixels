...
type Registry struct {
	...
	// furniturePolicy resolves external furniture capabilities for active rooms.
...
func (registry *Registry) Activate(snapshot Snapshot) (*Room, error) {
	...
	room.SetFurniturePolicy(registry.furniturePolicy)
...
// SetFurniturePolicy replaces the external policy for current and future rooms.
func (registry *Registry) SetFurniturePolicy(policy FurniturePolicy) {
	registry.mutex.Lock()
	registry.furniturePolicy = policy
	...
	for _, room := range rooms {
		room.SetFurniturePolicy(policy)
	}
}
...
func (registry *Registry) JoinSpectator(ctx context.Context, roomID int64, occupant Occupant) (Occupancy, error) {
	if !occupant.Valid() {
	...
	if previousID, indexed := registry.indexedRoom(occupant.PlayerID); indexed && previousID != roomID {
		registry.removeIndexedPlayer(ctx, occupant.PlayerID)
	}
	occupancy, err := room.JoinSpectator(occupant)
	if err != nil {
