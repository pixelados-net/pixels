...
type Snapshot struct {
	...
	// AllowPets reports whether pets may be placed in the room.
	AllowPets bool
	// AllowPetsEat reports whether placed pets may consume products in the room.
	AllowPetsEat bool
}
...
type Occupant struct {
	...
	// GroupID identifies the favorite social group or minus one.
	GroupID int64
	// GroupStatus stores the favorite social membership role or minus one.
	GroupStatus int32
	// GroupName stores the favorite social group title.
	GroupName string
	...
	JoinedAt time.Time
	// Spectator reports an invisible overflow occupant without a world unit.
	Spectator bool
}
...
// UpdatePetSettings refreshes live pet placement and feeding policy.
func (room *Room) UpdatePetSettings(allowPets bool, allowPetsEat bool) {
	room.mutex.Lock()
	room.snapshot.AllowPets = allowPets
	room.snapshot.AllowPetsEat = allowPetsEat
	room.mutex.Unlock()
}
