...
// TestRoomSpectatorDoesNotConsumeCapacity verifies overflow viewers stay invisible to occupancy.
func TestRoomSpectatorDoesNotConsumeCapacity(t *testing.T) {
	room, err := NewRoom(Snapshot{ID: 9, MaxUsers: 1})
	if err != nil {
		t.Fatal(err)
	}
	...
	spectator := occupantForTest(8)
	spectator.ConnectionID = "spectator"
	occupancy, err := room.JoinSpectator(spectator)
	if err != nil {
		t.Fatal(err)
	}
	stored, found := room.Occupant(8)
	if !found || !stored.Spectator || occupancy.Count != 1 || len(occupancy.PlayerIDs) != 2 {
		t.Fatalf("occupant=%#v occupancy=%#v", stored, occupancy)
	}
	if _, removed := room.Leave(8); !removed || room.Occupancy().Count != 1 {
		t.Fatal("spectator leave changed visible occupancy")
	}
}

// BenchmarkRoomSpectatorOccupancy measures mixed visible and spectator snapshots.
func BenchmarkRoomSpectatorOccupancy(b *testing.B) {
	room, _ := NewRoom(Snapshot{ID: 9, MaxUsers: 1})
	_, _ = room.Join(occupantForTest(7))
	spectator := occupantForTest(8)
	spectator.ConnectionID = "spectator"
	_, _ = room.JoinSpectator(spectator)
	b.ResetTimer()
	for range b.N {
		_ = room.Occupancy()
	}
