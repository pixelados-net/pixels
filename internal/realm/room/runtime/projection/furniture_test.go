...
// TestFloorItemsVisibleHidesOnlyConfigurationBoxes verifies game pieces remain visible.
func TestFloorItemsVisibleHidesOnlyConfigurationBoxes(t *testing.T) {
	x, y, z := 1, 1, 0.0
	items := []furnituremodel.Item{
		{DefinitionID: 10, OwnerPlayerID: 7, X: &x, Y: &y, Z: &z},
		{DefinitionID: 11, OwnerPlayerID: 7, X: &x, Y: &y, Z: &z},
		{DefinitionID: 12, OwnerPlayerID: 7, X: &x, Y: &y, Z: &z},
	}
	items[0].ID, items[1].ID, items[2].ID = 1, 2, 3
	definitions := map[int64]furnituremodel.Definition{
		10: {SpriteID: 10, InteractionType: "wf_trg_enter_room"},
		11: {SpriteID: 11, InteractionType: "wf_blob"},
		12: {SpriteID: 12, InteractionType: "wf_highscore_classic"},
	}
	owners, records := FloorItemsVisible(items, definitions, map[int64]string{7: "demo"}, nil, true)
	if len(owners) != 1 || len(records) != 2 || records[0].ID != 2 || records[1].ID != 3 {
		t.Fatalf("owners=%+v records=%+v", owners, records)
	}
}
