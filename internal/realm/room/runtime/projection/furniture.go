...
func FloorItems(items []furnituremodel.Item, definitions map[int64]furnituremodel.Definition, ownerNames map[int64]string, giftSenders map[int64]GiftSender) ([]outflooritems.Owner, []outflooritems.FloorItem) {
	return FloorItemsVisible(items, definitions, ownerNames, giftSenders, false)
}

// FloorItemsVisible maps placed furniture while optionally hiding WIRED configuration boxes.
func FloorItemsVisible(items []furnituremodel.Item, definitions map[int64]furnituremodel.Definition, ownerNames map[int64]string, giftSenders map[int64]GiftSender, hideWired bool) ([]outflooritems.Owner, []outflooritems.FloorItem) {
	visible := make([]furnituremodel.Item, 0, len(items))
	records := make([]outflooritems.FloorItem, 0, len(items))
	for _, item := range items {
		definition, ok := definitions[item.DefinitionID]
		if !ok || definition.Kind == furnituremodel.KindWall || item.X == nil || item.Y == nil || item.Z == nil || hideWired && isWiredBox(definition.InteractionType) {
			continue
		}
	...
	if hideWired {
		return ownerRecords(visible, ownerNames), records
	}
	return ownerRecords(items, ownerNames), records
}

// isWiredBox reports configuration-box interaction types without hiding game blobs or scoreboards.
func isWiredBox(interaction string) bool {
	return strings.HasPrefix(interaction, "wf_") && !strings.HasPrefix(interaction, "wf_blob") && !strings.HasPrefix(interaction, "wf_highscore")
}
