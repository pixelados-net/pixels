...
func TestStatusActionsKeepDanceOnItsDedicatedPacket(t *testing.T) {
	actions := statusActions(roomlive.UnitSnapshot{Statuses: []worldunit.Status{{Key: worldunit.StatusDance, Value: "3"}, {Key: worldunit.StatusSign, Value: "7"}}})
	if len(actions) != 1 || actions[0].Key != worldunit.StatusSign {
		t.Fatalf("unexpected actions %#v", actions)
	}
}

// TestStatusActionsRaiseMountedPendingMovement verifies late entrants receive the rider's elevated goal.
func TestStatusActionsRaiseMountedPendingMovement(t *testing.T) {
	actions := statusActions(roomlive.UnitSnapshot{
		Statuses: []worldunit.Status{{Key: worldunit.StatusMove, Value: "2,3,1.5"}}, RenderOffset: worldunit.RidingHeightOffset,
	})
	if len(actions) != 1 || actions[0].Value != "2,3,2.5" {
...
// TestRiderProjectionOffsetsSpawnAndMovement verifies the physical tile stays shared while Nitro renders the rider above the pet.
func TestRiderProjectionOffsetsSpawnAndMovement(t *testing.T) {
	unit := roomlive.UnitSnapshot{
		UnitID: 1, Position: worldpath.Position{Point: grid.MustPoint(1, 3), Z: grid.HeightFromInt(2)},
		Previous: worldpath.Position{Point: grid.MustPoint(1, 3), Z: grid.HeightFromInt(2)}, RenderOffset: worldunit.RidingHeightOffset,
	}
	status := statusPositionRecord(unit, unit.Position, nil)
	movements := MovementStatuses([]roomlive.Movement{{
		Unit: unit, Step: worldpath.Step{Position: worldpath.Position{Point: grid.MustPoint(2, 3), Z: grid.HeightFromInt(2)}}, Moved: true,
	}})
	if status.Z != "3" || len(movements) != 1 || movements[0].Z != "3" || movements[0].Actions[0].Value != "2,3,3" {
		t.Fatalf("status=%+v movements=%+v", status, movements)
	}
}
