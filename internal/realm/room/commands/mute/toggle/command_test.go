...
// roomFinderForTest returns one room.
type roomFinderForTest struct {
	// room stores the returned room.
	room roommodel.Room
}

// FindByID finds one room.
func (finder roomFinderForTest) FindByID(context.Context, int64) (roommodel.Room, bool, error) {
	return finder.room, true, nil
}

// permissionsForTest allows every requested node.
...
// eventsForTest captures one event.
...
func TestHandleTogglesBroadcastsAndPublishes(t *testing.T) {
	...
	authorizer := roomsettings.New(permissionsForTest{}, roomsettings.Nodes{OwnManage: "own", AnyManage: "any"})
	handler := Handler{Players: players, Bindings: bindings, Rooms: roomFinderForTest{room: room}, Authorize: authorizer, Runtime: runtime, Connections: connections, Events: events}
	err = handler.Handle(context.Background(), command.Envelope[Command]{Command: Command{Handler: netconn.Context{ConnectionID: "conn", ConnectionKind: "websocket"}}})
	if err != nil {
