...
func TestHandleMutatesAndPublishesFilter(t *testing.T) {
	...
	filters, events := &filtersForTest{}, &eventsForTest{}
	handler := Handler{Players: players, Bindings: bindings, Filters: filters, Events: events}
	err = handler.Handle(context.Background(), command.Envelope[Command]{Command: Command{Handler: netconn.Context{ConnectionID: "conn", ConnectionKind: "websocket"}, RoomID: 9, Add: true, Word: " Spam "}})
	if err != nil {
