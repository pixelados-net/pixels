...
// TestFixturesOpensOnlyBrokenFreezeBlocks verifies dynamic block collision state.
func TestFixturesOpensOnlyBrokenFreezeBlocks(t *testing.T) {
	definition := Definition{InteractionType: "freeze_block", Width: 1, Length: 1, StackHeight: 500, AllowStack: true}
	for _, test := range []struct {
		// name identifies the fixture case.
		name string
		// state stores the block's native furniture state.
		state string
		// want stores the expected collision state.
		want surface.State
	}{{"intact", "0", surface.StateBlocked}, {"armed delay", "11", surface.StateBlocked}, {"empty", "1000", surface.StateOpen}, {"reward", "7000", surface.StateOpen}, {"collected", "17000", surface.StateOpen}} {
		t.Run(test.name, func(t *testing.T) {
			fixtures, err := Fixtures(Item{ID: 4, Point: grid.MustPoint(1, 1), Definition: definition, ExtraData: test.state})
			if err != nil {
				t.Fatal(err)
			}
			assertFixture(t, fixtures[0], 0, 500, test.want, true, 4)
		})
	}
}
