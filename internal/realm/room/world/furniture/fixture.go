...
// freezeBlockOpen reports whether an exploded Freeze block exposes its floor tile.
func freezeBlockOpen(item Item) bool {
	if item.Definition.InteractionType != "freeze_block" {
		return false
	}
	state, err := strconv.Atoi(item.ExtraData)
	if err != nil {
		return false
	}
	visual := state / 1000
	return visual >= 1 && visual <= 7 || visual >= 12 && visual <= 17
}
