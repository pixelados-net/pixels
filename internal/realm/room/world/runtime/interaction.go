...
func (world *World) RollUnit(entityKey int64, position worldpath.Position) (UnitSnapshot, error) {
	...
	if linkedKey, linked := world.linkedKey(entityKey); linked {
		if linkedUnit, linkedFound := world.units[linkedKey]; linkedFound {
			linkedUnit.Reposition(position, roomUnit.BodyRotation())
		}
	}
...
func (world *World) ApplyControlledInteractionStep(playerID int64, point grid.Point, control worldunit.ControlKind) error {
	...
	if !grid.Adjacent(roomUnit.Position().Point, point) {
	...
	roomPath := worldpath.NewPath([]worldpath.Step{{Position: worldpath.Position{Point: point, Z: section.Z()}}})
...
func (world *World) FurnitureByInteraction(interactionType string) []worldfurniture.Item {
	items := worldindex.ByInteraction(world.interactionTypes[interactionType], world.furniture)
	sort.Slice(items, func(left int, right int) bool { return items[left].ID < items[right].ID })
	return items
}

// NearestFurnitureByInteraction returns the closest indexed item without allocating.
func (world *World) NearestFurnitureByInteraction(interactionType string, origin grid.Point, radius int) (worldfurniture.Item, bool) {
	return worldindex.Nearest(world.interactionTypes[interactionType], world.furniture, origin, radius)
}
