...
// UnitInFurnitureFootprint returns the lowest-key unit occupying an item's rotated footprint.
func (world *World) UnitInFurnitureFootprint(item worldfurniture.Item) (UnitSnapshot, bool) {
	...
	found := false
	for entityKey, roomUnit := range world.units {
		point := roomUnit.Position().Point
		x, y := int(point.X), int(point.Y)
		if x >= minX && x < maxX && y >= minY && y < maxY && (!found || entityKey < selectedKey) {
	...
	return unitSnapshot(selectedKey, world.units[selectedKey]), true
}
...
func unitSnapshot(playerID int64, roomUnit *worldunit.Unit) UnitSnapshot {
	...
	return UnitSnapshot{
		...
		Moving: roomUnit.InMotion(), Statuses: roomUnit.Statuses(), HandItem: roomUnit.HandItem(), Idle: roomUnit.Idle(), IdleSince: roomUnit.IdleSince(), ManualIdle: roomUnit.ManualIdle(), ActiveEffectID: roomUnit.ActiveEffect(), RenderOffset: roomUnit.RenderOffset(),
	}
}
