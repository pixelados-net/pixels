...
func (handler Handler) join(ctx context.Context, player *playerlive.Player, connection netconn.Context, room roommodel.Room, roomLayout layout.Layout) (*roomlive.Room, error) {
	...
	if handler.Groups != nil {
		occupant.GroupID, occupant.GroupStatus, occupant.GroupName = handler.Groups.Favorite(player.ID())
	}
	...
	if err == roomlive.ErrRoomFull && handler.Entry != nil {
		...
		} else if handler.Entry.Config().SpectatorEnabled {
			_, err = handler.Runtime.JoinSpectator(ctx, room.ID, occupant)
		}
	}
	if err != nil {
		return nil, err
