...
// TestSendRoomStateIncludesExistingEffects verifies late entrants receive current game teams.
func TestSendRoomStateIncludesExistingEffects(t *testing.T) {
	connection, sent := sessionConnectionForTest(t)
	room := activeRoomForBroadcastTest(t)
	if _, found := room.SetUnitEffect(7, 35); !found {
		t.Fatal("set existing team effect")
	}

	if err := (Handler{}).sendRoomState(context.Background(), connection, room, 0); err != nil {
		t.Fatalf("send room state: %v", err)
	}
	if len(*sent) != 3 {
		t.Fatalf("expected units, statuses, and effect packets, got %#v", *sent)
	}
	if packet := (*sent)[2]; packet.Header != outeffect.Header {
		t.Fatalf("effect header=%d want=%d", packet.Header, outeffect.Header)
	}
}
