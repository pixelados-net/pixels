...
func (store *auditStoreForTest) RightsHistory(_ context.Context, query auditrepo.Query) ([]auditmodel.RightsAudit, error) {
	store.query = query
	return store.rights, nil
}

// ModerationHistory returns configured moderation rows.
func (store *auditStoreForTest) ModerationHistory(_ context.Context, query auditrepo.Query) ([]auditmodel.ModerationAction, error) {
	store.query = query
