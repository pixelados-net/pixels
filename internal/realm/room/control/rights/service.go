...
func New(store Store, rooms RoomFinder, permissions permissionservice.Checker, events bus.Publisher, nodes Nodes) *Service {
...
// RoomIDsForPlayer lists rooms where a player holds explicit rights.
func (service *Service) RoomIDsForPlayer(ctx context.Context, playerID int64) ([]int64, error) {
	if playerID <= 0 {
		return nil, ErrInvalidIdentity
	}
	store, ok := service.store.(PlayerRoomStore)
	if !ok {
		return nil, ErrInvalidIdentity
	}
	return store.ListRoomIDsByPlayer(ctx, playerID)
}
