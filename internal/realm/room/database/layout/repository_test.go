...
func TestRepositoryCreateScansLayout(t *testing.T) {
	...
	roomLayout, err := NewRepository(executor).Create(context.Background(), roomlayout.CreateRecordParams{Layout: validLayoutForTest()})
	if err != nil {
...
func TestRepositoryUpdateReportsMissingLayout(t *testing.T) {
	...
	_, found, err := NewRepository(executor).Update(context.Background(), roomlayout.UpdateRecordParams{ID: 77, Layout: validLayoutForTest()})
	if err != nil {
...
func validLayoutForTest() roomlayout.Layout {
	return roomlayout.Layout{Name: "model_a", TileSize: 12, Heightmap: "xxx\rx0x\rxxx", DoorX: 1, DoorY: 1, DoorDirection: 2, Enabled: true}
}
