...
func TestRepositoryReadsAuditRows(t *testing.T) {
	...
	repository := &Repository{executor: executor}
	rights, err := repository.RightsHistory(context.Background(), roomaudit.StoreQuery{Limit: 50})
	...
	actions, err := repository.ModerationHistory(context.Background(), roomaudit.StoreQuery{Limit: 50, ActionTypes: []moderationmodel.Action{moderationmodel.ActionKick}})
