...
func (service *Service) Handle(ctx context.Context, request Request) error {
	...
	pluginPlayer := sdkPlayer(player)
	if service.commandExecutor != nil {
		handled, commandErr := service.commandExecutor.Execute(ctx, pluginPlayer, message)
	...
	if service.eventDispatcher != nil {
		var cancelled bool
		message, cancelled = service.eventDispatcher.DispatchChat(ctx, pluginPlayer, roomID, message)
		message = sanitize(message)
		if cancelled || message == "" {
			return nil
		}
	}
	if request.Kind != KindWhisper && service.speechInterceptor != nil {
		consumed, interceptErr := service.speechInterceptor.Intercept(ctx, roomID, player.ID(), player.Username(), message)
...
// sdkPlayer creates the immutable plugin-facing speaker snapshot.
func sdkPlayer(player interface {
	ID() int64
	Username() string
	CurrentRoom() (int64, bool)
}) sdkplayer.Player {
	roomID, _ := player.CurrentRoom()
	return sdkplayer.Player{ID: player.ID(), Username: player.Username(), RoomID: roomID, Online: true}
}
