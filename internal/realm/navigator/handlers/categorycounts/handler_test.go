...
import (
	...
	incounts "github.com/niflaot/pixels/networking/inbound/navigator/categorycounts"
	outcounts "github.com/niflaot/pixels/networking/outbound/navigator/categorycounts"
	"go.uber.org/zap"
)

// TestHandlerSendsCategoryCounts verifies category count request handling.
func TestHandlerSendsCategoryCounts(t *testing.T) {
	sent := make([]codec.Packet, 0, 1)
	session := testSession(t, New(countscmd.Handler{
		Players:  testPlayers(t),
		Bindings: testBindings(t),
		Counts:   testCounts{},
	}, zap.NewNop()), &sent)

	if err := session.Receive(context.Background(), codec.Packet{Header: incounts.Header}); err != nil {
		t.Fatalf("receive category counts: %v", err)
	}
	if len(sent) != 1 || sent[0].Header != outcounts.Header {
		t.Fatalf("unexpected sent packets %#v", sent)
	}
}

// testCounts provides category count snapshots for handler tests.
type testCounts struct{}

// Snapshot returns category count entries for handler tests.
func (testCounts) Snapshot() []outcounts.Entry {
	return []outcounts.Entry{{CategoryID: 7, CurrentVisitorCount: 2, MaxVisitorCount: 25}}
}
