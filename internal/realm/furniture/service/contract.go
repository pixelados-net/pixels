...
// RoomGranter creates furniture and places it directly into a room.
type RoomGranter interface {
	DefinitionFinder
	Granter

	// Place moves a newly granted item into a room.
...
// StackHeightUpdater changes exact per-item stacking surfaces.
type StackHeightUpdater interface {
	// UpdateStackHeight changes one placed item's exact surface override.
	UpdateStackHeight(ctx context.Context, itemID int64, roomID int64, heightCM *int32) (furnituremodel.Item, error)
}
...
// roomGranterAssertion verifies Service implements RoomGranter.
var roomGranterAssertion RoomGranter = (*Service)(nil)
