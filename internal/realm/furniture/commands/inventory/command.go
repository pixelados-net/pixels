...
type Handler struct {
	...
	// Groups resolves warmed social-group furniture identity.
...
func (handler Handler) sendFragments(ctx context.Context, connection netconn.Context, items []furnituremodel.Item, definitions map[int64]furnituremodel.Definition) error {
	...
	for fragment := 0; fragment < totalFragments; fragment++ {
		packet, err := outlist.Encode(fragment+1, totalFragments, fragmentRecords(items, definitions, handler.Groups, fragment))
		if err != nil {
			return err
		}
...
func fragmentRecords(items []furnituremodel.Item, definitions map[int64]furnituremodel.Definition, groups furnituremodel.GroupPolicy, fragment int) []outlist.Item {
	...
	for _, item := range items[start:end] {
		...
		record := outlist.Item{
			...
			AllowRecycle:         definition.AllowRecycle,
		...
		if groups != nil {
			if group, found := groups.Furniture(item.ID); found {
				record.Data = stuffdata.StringArray([]string{
					item.ExtraData,
					strconv.FormatInt(group.GroupID, 10),
					group.BadgeCode,
					group.ColorAHex,
					group.ColorBHex,
				})
