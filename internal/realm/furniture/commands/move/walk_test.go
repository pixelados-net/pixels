...
// TestHandleStopsDecoratorRoute verifies furniture manipulation cannot continue avatar walking.
func TestHandleStopsDecoratorRoute(t *testing.T) {
	...
	if _, err := active.MoveTo(7, grid.MustPoint(3, 0)); err != nil {
	...
	if err := handler.Handle(context.Background(), command.Envelope[Command]{
		...
		t.Fatalf("move furniture: %v", err)
	}
	settled := active.Tick()
	if len(settled) != 1 || !settled[0].Settled || settled[0].Moved || settled[0].Unit.Moving {
		t.Fatalf("expected decorator to settle without advancing, got %#v", settled)
	}
	if settled[0].Unit.Position.Point != grid.MustPoint(0, 0) {
		t.Fatalf("decorator advanced to %#v", settled[0].Unit.Position)
	}
}
