...
import (
	...
	surfaceevent "github.com/niflaot/pixels/internal/realm/furniture/events/surfaceapplied"
...
func (handler Handler) handleSurface(ctx context.Context, player *playerlive.Player, active *roomlive.Room, roomID int64, command Command) error {
	...
	if handler.Events != nil {
		_ = handler.Events.Publish(ctx, bus.Event{Name: surfaceevent.Name, Payload: surfaceevent.Payload{PlayerID: player.ID(), RoomID: roomID, Surface: string(surface)}})
	}
