...
func addRecord(item furnituremodel.Item, definition furnituremodel.Definition, ownerName string, sender projection.GiftSender, group furnituremodel.GroupData, linked bool) outadd.FloorItem {
	...
	if linked {
		record.Data = stuffdata.StringArray([]string{item.ExtraData, strconv.FormatInt(group.GroupID, 10), group.BadgeCode, group.ColorAHex, group.ColorBHex})
	}
