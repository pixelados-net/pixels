...
const (
	...
	itemColumns = `id, definition_id, owner_player_id, room_id, x, y, z::float8, rotation, wall_position, extra_data, rental_owner_player_id, rental_expires_at, rental_price_credits, stack_height_override_cm, limited_edition_number, marketplace_reserved, gift_wrapped, gift_wrap_sprite_id, gift_wrap_box_id, gift_wrap_ribbon_id, gift_sender_player_id, gift_message, metadata, created_at, updated_at, deleted_at, version`
...
returning ` + itemColumns
	...
	// updateItemStackHeightSQL changes one placed item's exact surface override.
	updateItemStackHeightSQL = `
update furniture_items
set stack_height_override_cm = $3, updated_at = now(), version = version + 1
where id = $1 and room_id = $2 and deleted_at is null
returning ` + itemColumns
