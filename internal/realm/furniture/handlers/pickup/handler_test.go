...
// TestRegisterWallAddsDedicatedRemovalHandler verifies Nitro's wall-item regression bridge.
func TestRegisterWallAddsDedicatedRemovalHandler(t *testing.T) {
	registry := netconn.NewHandlerRegistry()
	RegisterWall(registry, NewWall(pickupcmd.Handler{}, nil))
	packet, err := codec.NewPacket(inremovewall.Header, inremovewall.Definition, codec.Int32(1))
	if err != nil {
		t.Fatal(err)
	}
