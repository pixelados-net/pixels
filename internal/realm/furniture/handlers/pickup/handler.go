...
import (
	...
	inremovewall "github.com/niflaot/pixels/networking/inbound/furniture/removewall"
	"go.uber.org/zap"
)
...
// NewWall creates Nitro's dedicated wall-item removal adapter.
func NewWall(handler pickupcmd.Handler, log *zap.Logger) netconn.Handler {
	dispatcher, _ := command.NewDispatcher(handler)
	dispatcher.WithLogger(log)
	return func(connection netconn.Context, packet codec.Packet) error {
		itemID, err := inremovewall.Decode(packet)
		if err != nil {
			return err
		}
		return dispatcher.Dispatch(context.Background(), command.Envelope[pickupcmd.Command]{
			Command:  pickupcmd.Command{Handler: connection, ItemID: int64(itemID)},
			Metadata: command.Metadata{ConnectionID: string(connection.ConnectionID)},
		})
	}
}
...
// RegisterWall adds Nitro's dedicated wall-item removal header.
func RegisterWall(registry *netconn.HandlerRegistry, handler netconn.Handler) {
	_ = registry.Register(inremovewall.Header, handler)
}
