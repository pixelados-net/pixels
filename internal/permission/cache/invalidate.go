...
func (cache *Cache) delete(ctx context.Context, key string) {
	cache.removeLocal(key)
	if cache.redis != nil {
		if err := cache.redis.Delete(ctx, key); err != nil {
			cache.log.Warn("permission shared cache invalidation failed", zap.String("key", key), zap.Error(err))
		}
	}
}

// removeLocal removes one process-local key.
func (cache *Cache) removeLocal(key string) {
	cache.mutex.Lock()
