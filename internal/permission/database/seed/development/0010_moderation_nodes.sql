...
select id,'moderation.guide.duty',true from players where lower(username)='bob'
on conflict(player_id,node) do update set allowed=excluded.allowed;

insert into permission_player_nodes(player_id,node,allowed)
select id,'moderation.guardian.duty',true from players where lower(username)='carol'
on conflict(player_id,node) do update set allowed=excluded.allowed;
