...
func TestDecodeWithoutOptionalIssue(t *testing.T) {
	packet, err := codec.NewPacket(Header, Definition, codec.Int32(2), codec.String("reason"), codec.Int32(4), codec.Int32(3), codec.Bool(false))
	if err != nil {
		t.Fatal(err)
	}
	payload, err := Decode(packet)
	if err != nil || payload.TopicID != 4 || payload.ActionIndex != 3 || payload.AvatarOnly || payload.IssueID != 0 {
...
func TestDecodeWithOptionalIssue(t *testing.T) {
	packet, err := codec.NewPacket(Header, Definition, codec.Int32(2), codec.String("reason"), codec.Int32(4), codec.Int32(7), codec.Bool(true), codec.Int32(9))
	if err != nil {
		t.Fatal(err)
	}
	payload, err := Decode(packet)
	if err != nil || payload.IssueID != 9 || !payload.AvatarOnly {
		t.Fatalf("payload=%+v err=%v", payload, err)
	}
}
