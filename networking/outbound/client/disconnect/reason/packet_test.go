...
// TestEncode verifies DISCONNECT_REASON packet construction.
...
// TestEncodeWithOptions verifies optional DISCONNECT_REASON packet fields.
func TestEncodeWithOptions(t *testing.T) {
	packet, err := Encode(WithReason(7))
	if err != nil {
		t.Fatalf("encode packet: %v", err)
	}
