...
func friendsHandler(dependencies Dependencies) fiber.Handler {
	return func(ctx *fiber.Ctx) error {
		...
		for index, card := range cards {
			...
			friends[index] = FriendResponse{PlayerID: card.ID, Username: card.Username, Look: card.Look, Gender: gender, Online: card.Online, InRoom: card.FollowingAllowed, Relation: int16(card.Relation)}
		}
