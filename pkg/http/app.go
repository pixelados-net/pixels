...
func New(log *zap.Logger, config config.AppConfig, info build.Info, sso *sso.Service, websocket *ws.Adapter, rooms roomservice.Manager, runtime *roomlive.Registry, roomEntry *roomentry.Service, navigator navservice.Manager, currencyAdmin currencyroutes.Dependencies, catalogAdmin catalogroutes.Dependencies) *fiber.App {
	return newApplication(log, config, info, sso, websocket, rooms, runtime, roomEntry, navigator, currencyAdmin, catalogAdmin, permissionroutes.Dependencies{}, roomroutes.Dependencies{})
}

// NewWithPermissions creates the complete Fiber application.
func NewWithPermissions(log *zap.Logger, config config.AppConfig, info build.Info, sso *sso.Service, websocket *ws.Adapter, rooms roomservice.Manager, runtime *roomlive.Registry, roomEntry *roomentry.Service, navigator navservice.Manager, currencyAdmin currencyroutes.Dependencies, catalogAdmin catalogroutes.Dependencies, permissionAdmin permissionroutes.Dependencies, roomAdmin roomroutes.Dependencies) *fiber.App {
	return newApplication(log, config, info, sso, websocket, rooms, runtime, roomEntry, navigator, currencyAdmin, catalogAdmin, permissionAdmin, roomAdmin)
}
...
func newApplication(log *zap.Logger, config config.AppConfig, info build.Info, sso *sso.Service, websocket *ws.Adapter, rooms roomservice.Manager, runtime *roomlive.Registry, roomEntry *roomentry.Service, navigator navservice.Manager, currencyAdmin currencyroutes.Dependencies, catalogAdmin catalogroutes.Dependencies, permissionAdmin permissionroutes.Dependencies, roomAdmin roomroutes.Dependencies) *fiber.App {
	...
	registerPrivate(app, sso, rooms, runtime, roomEntry, navigator, currencyAdmin, catalogAdmin, permissionAdmin, roomAdmin)
