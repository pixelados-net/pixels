...
type UIConfigResponse struct {
	...
	// RecyclerBatchSize keeps Nitro's selection limit aligned with server policy.
	RecyclerBatchSize int `json:"crafting.recycler.batch.size"`
}
...
func uiConfigHandler(currencies currencyservice.Reader, layouts roomlayout.Manager) fiber.Handler {
	return func(ctx *fiber.Ctx) error {
		...
		return ctx.JSON(UIConfigResponse{CurrencyTypes: types, RoomModels: models, RecyclerBatchSize: craftingconfig.Load().RecyclerBatchSize})
	}
}
